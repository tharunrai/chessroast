...
export class MockAnalysisProvider implements AnalysisProvider {
  async analyzeGame(pgn: string, playerColor: 'white' | 'black' = 'white', opponentName: string = 'Opponent'): Promise<GameAnalysisResult> {
    ...
    const systemPrompt = `You are Stockfish AI Roast Game Reviewer: an elite chess engine and stand-up comedian. Write the report in second person and speak directly to the user with you/your language only. Return ONLY valid JSON.`;
...
Generate a humorous Stockfish Game Review. Write everything as if you are roasting the user directly. Keep the voice direct and conversational, but never use first-person language. Return JSON matching this schema:
{
  "gameSummary": "Hilarious 2-sentence summary of how your match unfolded and collapsed",
  "biggestBlunders": [
    "Funny quote roasting your biggest blunder moment",
    "Funny quote roasting your endgame or time management"
  ],
  "finalVerdict": "Cooked 🔥",
  "roastScore": 9.1,
  "moveHighlights": [
    { "moveNumber": 12, "san": "${sampleMoves[0]?.split(': ')[1] || 'Qd2'}", "comment": "Hilarious joke about this move aimed at you", "type": "blunder" },
    { "moveNumber": 16, "san": "${sampleMoves[1]?.split(': ')[1] || 'Kh1'}", "comment": "Another funny joke about your choice", "type": "mistake" },
    { "moveNumber": 22, "san": "Resigns", "comment": "The best move you played all game.", "type": "brilliant" }
  ]
}`;
  ...
  private getOfflineFallback(opponentName: string, moveCount: number, sampleMoves: string[]): GameAnalysisResult {
      return {
        gameSummary: `🤖 AI Game Review Engine Offline: Your position survived ${moveCount} ply against ${opponentName} (with moves like ${sampleMoves.slice(0, 2).join(', ')}). To unlock live AI move-by-move roast analysis, please set GITHUB_TOKEN in .env.local!`,
